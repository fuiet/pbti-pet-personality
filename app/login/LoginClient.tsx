...
export default function LoginClient() {
  ...
  async function handleGoogleLogin() {
    ...
    try {
      ...
      setError(zh ? "暂时无法连接 Google 登录，请稍后重试。" : authError instanceof Error ? authError.message : "Unable to start Google sign-in.");
    }
  }
  ...
  async function handleEmailLogin(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(zh ? "邮箱或密码有误，请检查后重试。" : authError instanceof Error ? authError.message : "Unable to sign in with email.");
    }
  }
      ...
      <div className="w-full max-w-sm">
        <div className="rounded-3xl border border-[#eaded2] bg-white p-8 shadow-sm">
          <div className="mb-6 text-center">
            ...
            <h1 className="mt-3 text-2xl font-black tracking-[-.03em] text-[#171514]">{zh ? "欢迎回来" : "Welcome Back"}</h1>
            <p className="mt-1 text-sm text-[#7a6d63]">{zh ? "使用 Google 或 PBTI 账户登录" : "Use Google or your PBTI account to sign in"}</p>
          </div>
          ...
          <div className="space-y-3">
            ...
            >
              ...
              {isGoogleSubmitting ? (zh ? "正在连接…" : "Connecting...") : (zh ? "使用 Google 登录" : "Continue with Google")}
            </button>
            ...
            <form onSubmit={handleEmailLogin} className="space-y-3">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-2xl border border-[#eaded2] bg-white px-4 py-3 text-sm outline-none transition focus:border-[#ffb273] focus:ring-2 focus:ring-[#ffe2cb]"
                  placeholder={zh ? "请输入密码" : "Your password"}
                  required
                />
              </div>
              ...
              >
                {isEmailSubmitting ? (zh ? "正在登录…" : "Signing in...") : (zh ? "使用邮箱登录" : "Sign in with Email")}
              </button>
            </form>
          </div>

          <p className="mt-5 text-center text-sm text-[#7a6d63]">
            {zh ? "还没有账户？" : "Need a new account? "}<a href="/register" className="font-bold text-[#ff7a1a] hover:underline">{zh ? "立即注册" : "Register"}</a>
          </p>
        </div>
      </div>
    </div>
